...
use hyper::{body::{Bytes, Incoming}, server::conn::http1, HeaderMap, Request, Response};
...
async fn hello(req: Request<Incoming>) -> Result<Response<Full<Bytes>>, String> {
    ...
    let res = json.iter().find(|e| {
        req.uri().path() == e.url && e.method.eq_ignore_ascii_case(req.method().as_str())
